...
describe('AerolineaAeropuertoService', () => {
  ...
  it('deleteAirportsFromAirline should thrown an exception for an invalid airport', async () => {
    await expect(()=> service.deleteAirportsFromAirline(aerolinea.id, "0")).rejects.toHaveProperty("message", "The aeropuerto with the given id was not found"); 
  });

  it('deleteAirportsFromAirline should thrown an exception for an invalid airport', async () => {
    const aeropuerto: AeropuertoEntity = aeropuertosList[0];
    await expect(()=> service.deleteAirportsFromAirline("0", aeropuerto.id)).rejects.toHaveProperty("message", "The aerolinea with the given id was not found"); 
  });
  ...
  it('deleteAirportsFromAirline should thrown an exception for an non asocciated aeropuerto', async () => {
    ...
    await expect(()=> service.deleteAirportsFromAirline(aerolinea.id, newAeropuerto.id)).rejects.toHaveProperty("message", "The aeropuerto with the given id is not associated to the aerolinea"); 
  }); 
